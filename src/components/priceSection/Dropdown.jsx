           ...
           <div className="mt-4">
                        {
                            DataDropDown.map((item,index)=> {
                                return (
                                    <>
                                        <Wrap key={index} onClick={()=> toggle(index) }> {item.SectionLesson}  <Span>{ click === index  ? <IoIosArrowUp/> : <IoIosArrowDown/> }</Span>   </Wrap>
                                        { 
                                            click  === index ?  (
                                                <WraperDropDown>{
                                                    item.SectionPrices.map((e,i)=>{
                                                        return (
                                                            <>
                                                                <p key={i} >{e.nro} lesson / <Small> monthly </Small>  <Price>USD   {e.price} </Price> </p>
