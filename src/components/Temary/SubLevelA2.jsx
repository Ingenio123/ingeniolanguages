...
export default function SubLevelA2({subLevel,Level}) {
    ...
    const createTemary = (data)=>{
        ...
        document.getElementById("form_temary2").reset();
        
    }

    return (
        <div>
            <SubLevelItem className="text-center">{subLevel? subLevel:<LevelMuestra>Level</LevelMuestra>}</SubLevelItem>
            <Form onSubmit={ handleSubmit(createTemary) } id="form_temary2">
                    <GridForm>
                        <ItemsContent>
                            <label >Item Content 1</label>
                            <InputFormContent type="text"    {...register("item1",{
                        ...
                        <UrlExamen>
                            <label htmlFor="UrlExam">{subLevel === 'A2.3'? <UrlExamFinally>Url Exam Finally</UrlExamFinally>: 'url exam'}</label>
...
const Input = css`
    padding:4px 10px;
    border:1px solid rgba(165, 94, 234,.3);
    ...
    &:focus{
        border: 1px solid rgba(75, 123, 236,.5);
        box-shadow: 0 1px 1px rgba(0,0,0,0.075),0 0 2px rgba(165, 94, 234,.2);
    }
...
const InputFormContent =  styled.input `
    ${Input};
`
...
const LevelMuestra =  styled.span `
    ...
    font-weight: 200;
`
const UrlExamFinally =  styled.span`
    color:red;
`